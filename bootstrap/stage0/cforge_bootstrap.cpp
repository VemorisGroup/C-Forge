...
static Value cfv_sha256_range(const Value& bytes_value,
    ...
    // SHA-256 portable (FIPS 180-4) – Apple, Linux y Windows
    static const std::uint32_t cfv_K256[64] = {
    ...
    std::uint32_t ha=0x6a09e667u,hb=0xbb67ae85u,hc=0x3c6ef372u,hd=0xa54ff53au;
    std::uint32_t he=0x510e527fu,hf=0x9b05688cu,hg=0x1f83d9abu,hh=0x5be0cd19u;
    const std::size_t mlen256 = ((input.size()+72u)/64u)*64u;
    std::vector<unsigned char> msg256(mlen256, 0u);
    std::copy(input.begin(), input.end(), msg256.begin());
    msg256[input.size()] = 0x80u;
    const std::uint64_t bl256 = static_cast<std::uint64_t>(input.size())*8u;
    for (int bi=0; bi<8; ++bi)
        msg256[mlen256-1-static_cast<std::size_t>(bi)]=static_cast<unsigned char>((bl256>>(8*bi))&0xffu);
    for (std::size_t ck=0; ck<mlen256; ck+=64u) {
        std::uint32_t w[64];
        for (int i=0; i<16; ++i)
            w[i]=(static_cast<std::uint32_t>(msg256[ck+static_cast<std::size_t>(i)*4u])<<24u)
                |(static_cast<std::uint32_t>(msg256[ck+static_cast<std::size_t>(i)*4u+1u])<<16u)
                |(static_cast<std::uint32_t>(msg256[ck+static_cast<std::size_t>(i)*4u+2u])<<8u)
                | static_cast<std::uint32_t>(msg256[ck+static_cast<std::size_t>(i)*4u+3u]);
        for (int i=16; i<64; ++i) {
            const std::uint32_t s0=((w[i-15]>>7u)|(w[i-15]<<25u))^((w[i-15]>>18u)|(w[i-15]<<14u))^(w[i-15]>>3u);
            const std::uint32_t s1=((w[i-2]>>17u)|(w[i-2]<<15u))^((w[i-2]>>19u)|(w[i-2]<<13u))^(w[i-2]>>10u);
            w[i]=w[i-16]+s0+w[i-7]+s1;
        }
        std::uint32_t a=ha,b=hb,c=hc,d=hd,e=he,f=hf,g=hg,h=hh;
        for (int i=0; i<64; ++i) {
            const std::uint32_t S1=((e>>6u)|(e<<26u))^((e>>11u)|(e<<21u))^((e>>25u)|(e<<7u));
            const std::uint32_t ch=(e&f)^(~e&g);
            const std::uint32_t t1=h+S1+ch+cfv_K256[i]+w[i];
            const std::uint32_t S0=((a>>2u)|(a<<30u))^((a>>13u)|(a<<19u))^((a>>22u)|(a<<10u));
            const std::uint32_t maj=(a&b)^(a&c)^(b&c);
            h=g; g=f; f=e; e=d+t1; d=c; c=b; b=a; a=t1+S0+maj;
        }
        ha+=a; hb+=b; hc+=c; hd+=d; he+=e; hf+=f; hg+=g; hh+=h;
    }
    unsigned char digest256[32];
    const auto cfv_pu32=[&](int base, std::uint32_t v) noexcept {
        digest256[base]  =static_cast<unsigned char>((v>>24u)&0xffu);
        digest256[base+1]=static_cast<unsigned char>((v>>16u)&0xffu);
        digest256[base+2]=static_cast<unsigned char>((v>>8u)&0xffu);
        digest256[base+3]=static_cast<unsigned char>(v&0xffu);
    };
    cfv_pu32(0,ha); cfv_pu32(4,hb); cfv_pu32(8,hc); cfv_pu32(12,hd);
    cfv_pu32(16,he); cfv_pu32(20,hf); cfv_pu32(24,hg); cfv_pu32(28,hh);
    std::vector<Value> result;
    result.reserve(32);
    for (const unsigned char byte : digest256)
